...
def get_current_baseline(num_qubits, iters):
    data = {
        "circuit_name": "GHZ",
        "simulator": "SQLITE NO UDF",
        "num_qbits": num_qubits,
        "iterations": iters,
        "time": None,
        "memory_peak": None,
    }
...
def plot_save_data_ghz():
    ...
    with open(
        f"./new_data/ghz_{SYSTEM}_{MAX_QUBITS}_{ITER}_median.json", "w"
    ) as outfile:
...
if __name__ == "__main__":
    ...
    # data = get_current_baseline(MAX_QUBITS, ITER)
    # save_data_to_file(
    #     data,
    #     f"baselines/{today}/baseline_densesvd_{SYSTEM}_{MAX_QUBITS}_{ITER}.json",
    # )
    ...
    data = open(
        "./new_data/baselines/28-04-2025/baseline_densesvd_PC-2080s_100_100.json"
    )

    compare_current_baseline(json.load(data))
